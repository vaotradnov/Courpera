...
def test_student_grades_page_lists_percent(db):
    t = User.objects.create_user(username="teach_gpg", password="pw")
    t.profile.role = "teacher"
    t.profile.save(update_fields=["role"])
    s = User.objects.create_user(username="stud_gpg", password="pw")
    ...
    c = Client()
    assert c.login(username="stud_gpg", password="pw")
