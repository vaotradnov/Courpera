...
def test_gradebook_csv_downloads_and_has_headers(db):
    ...
    s1.profile.role = "student"; s1.profile.save(update_fields=["role"])
    ...
    a = Assignment.objects.create(course=c, type=AssignmentType.QUIZ, title="Quiz 1", is_published=True, available_from=timezone.now())
    ...
    Grade.objects.create(assignment=a, course=c, student=s1, attempt=att1, achieved_marks=80.0, max_marks=100.0, released_at=timezone.now())
    att2 = Attempt.objects.create(assignment=a, student=s2, score=60.0)
    Grade.objects.create(assignment=a, course=c, student=s2, attempt=att2, achieved_marks=60.0, max_marks=100.0, released_at=timezone.now())
