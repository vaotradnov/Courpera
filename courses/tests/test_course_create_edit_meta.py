...
def test_teacher_can_create_with_meta_fields(db):
    ...
    c = Client()
    assert c.login(username="teach_meta_c", password="pw")
    resp = c.post(
        "/courses/create/",
        {
            "title": "MetaCourse",
...
def test_teacher_can_edit_meta_fields(db):
    ...
    course = Course.objects.create(
        owner=t, title="EditMe", description="", subject="AI", level="beginner", language="English"
    )
    c = Client()
    assert c.login(username="teach_meta_e", password="pw")
    r = c.post(
